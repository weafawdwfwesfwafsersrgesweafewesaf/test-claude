...
sc.render.engine = 'CYCLES'; sc.cycles.samples = 20; sc.cycles.use_denoising = False
...
# décors relus DANS LE FICHIER FINAL (Workspace.DecorMondes)
for p in json.load(open('/tmp/claude-0/rbx/deco.json')):
    if p['t'] != f'Level{lvl}':
        continue
    m = p['m']
    right, up = (m[0], m[3], m[6]), (m[1], m[4], m[7])
    neon = p['col'] and False
    add_box(rbx_matrix(p['p'], right, up), p['s'], mat(p['col'], p.get('mat') == 288), {0: 'S', 2: 'C'}.get(p.get('sh'), 'B'))
